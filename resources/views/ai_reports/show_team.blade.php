...
@section('content')
    <div class="space-y-8 max-w-7xl mx-auto">
        ...
        <div class="bg-gradient-to-br from-indigo-800 via-purple-800 to-slate-900 py-20 px-6 mb-12 rounded-3xl text-center shadow-2xl">
            <h1 class="text-6xl font-extrabold text-white tracking-tight mb-4">Team AI Analysis Report</h1>
            <p class="text-indigo-100 text-lg max-w-2xl mx-auto">Deep intelligence for team <span class="text-indigo-300 font-semibold">{{ $team->name }}</span> based on combined tasks and member scores.</p>
            <div class="flex gap-3 flex-wrap justify-center mt-8">
                ...
                <form action="{{ route('ai.report.generate_team', $team) }}" method="POST">
                    @csrf
                    <button type="submit"
        ...
        <div class="grid gap-8 lg:grid-cols-[1.2fr_1fr]">
            <!-- Left: Team Profile & Stats -->
            <section class="glass border border-slate-700/50 rounded-3xl overflow-hidden shadow-2xl bg-[#0e1018] p-8">
                <!-- Profile Header -->
                <div class="border-b border-slate-700/30 pb-6 mb-6">
                    <div class="flex items-center gap-5">
                        ...
                        <div>
                            ...
                            <p class="text-indigo-400 font-semibold text-sm mt-1">Project Team Evaluation</p>
                            @if($team->description)
                                <p class="text-slate-400 text-xs mt-1">{{ $team->description }}</p>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <!-- Team Leader Details -->
                <div class="space-y-4 border-b border-slate-700/20 pb-6 mb-6">
                    <div class="flex items-center justify-between">
                        <span class="text-slate-400 text-sm">👑 Team Lead</span>
                        <span class="text-white font-semibold text-sm">{{ $team->lead->name ?? 'None' }}</span>
                    </div>
                </div>

                <!-- Team Statistics -->
                <div>
                    <h3 class="text-lg font-bold text-white mb-6">📊 Team Statistics</h3>
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div
                            ...
                            <p class="text-xs uppercase tracking-[0.15em] font-semibold text-blue-300">Team Members</p>
                            <p class="mt-3 text-4xl font-black text-blue-400">{{ $metrics['members_count'] }}</p>
                            <p class="mt-1 text-xs text-slate-400">active engineers</p>
                        </div>
                        <div
                            ...
                            <p class="text-xs uppercase tracking-[0.15em] font-semibold text-emerald-300">Completed Tasks</p>
                            <p class="mt-3 text-4xl font-black text-emerald-400">{{ $metrics['completed_tasks'] }}</p>
                            <p class="mt-1 text-xs text-slate-400">team level tasks</p>
                        </div>
                        <div
                            ...
                            <p class="text-xs uppercase tracking-[0.15em] font-semibold text-purple-300">Completion Rate</p>
                            <p class="mt-3 text-4xl font-black text-purple-400">{{ number_format($metrics['team_completion_rate'] * 10, 1) }}<span
                                    class="text-2xl">%</span></p>
                            <p class="mt-1 text-xs text-slate-400">score: {{ number_format($metrics['team_completion_rate'], 2) }}/10</p>
                        </div>
                        <div
                            ...
                            <p class="text-xs uppercase tracking-[0.15em] font-semibold text-amber-300">Productivity Score</p>
                            <p class="mt-3 text-4xl font-black text-amber-400">{{ number_format($metrics['productivity_score'], 1) }}</p>
                            <p class="mt-1 text-xs text-slate-400">out of 10</p>
                        </div>
                        <div
                            ...
                            <p class="text-xs uppercase tracking-[0.15em] font-semibold text-teal-300">Team Collaboration</p>
                            <p class="mt-3 text-4xl font-black text-teal-400">{{ number_format($metrics['team_collaboration'], 1) }}</p>
                        ...
                        <div
                            ...
                            <p class="text-xs uppercase tracking-[0.15em] font-semibold text-rose-300">Leadership Score</p>
                            <p class="mt-3 text-4xl font-black text-rose-400">{{ number_format($metrics['leadership_score'], 1) }}</p>
                            <p class="mt-1 text-xs text-slate-400">out of 10</p>
                        </div>
                    </div>
                </div>
            ...
            <section class="glass border border-slate-700/50 rounded-3xl overflow-hidden shadow-2xl bg-[#0e1018]">
                @if($report)
                    <!-- Report Header -->
                    ...
                    <div class="p-8 space-y-6">
                        ...
                        <div
                            ...
                            <p class="text-slate-300 leading-relaxed text-base">{{ $report->summary }}</p>
                        </div>
                ...
                @else
                    ...
                    <div class="px-8 py-16 flex flex-col items-center justify-center gap-4">
                        ...
                        <p class="text-slate-500 text-sm text-center max-w-xs">Generate an AI-powered analysis by clicking the
                            button above to get personalized insights and recommendations for the team.</p>
                    </div>
                @endif
            </section>
        </div>
