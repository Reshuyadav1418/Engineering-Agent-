...
<div class="page-container animate-fade-up" style="max-width:800px;">
    ...
    <div style="display:flex; align-items:center; gap:14px; margin-bottom:28px;">
        ...
        <div>
            <h1 class="page-title">Create Sprint Task</h1>
            <p class="page-subtitle">Configure a new sprint task, choose assignment type, and define metrics.</p>
        </div>
    </div>

    {{-- Form Card --}}
    <div class="form-section" x-data="{
        assignmentType: '{{ old('assignment_type', 'individual') }}',
        members: @js(old('members', [])),
        teamMembersMap: @js($teamMembersMap),
        ...
        <form action="{{ route('tasks.store') }}" method="POST" style="display:flex; flex-direction:column; gap:22px;">
            @csrf
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                    placeholder="e.g. Set up callback routes, secure keys using env files, and test login flows..."
            ...
            <div x-show="assignmentType === 'team'" x-transition style="display:flex; flex-direction:column; gap:16px;">
                <div>
                    ...
                    <select name="team_id" id="team_id" :required="assignmentType === 'team'" class="form-input" style="cursor:pointer;"
                        @change="loadTeamMembers($event.target.value)">
                        <option value="" disabled selected style="color:#000;">Select team…</option>
                        @foreach($teams as $team)
                            <option value="{{ $team->id }}" {{ old('team_id') == $team->id ? 'selected' : '' }} style="color:#000;">
                ...
                <div>
                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:12px;">
                        <label class="form-label" style="margin-bottom:0;">Team Members & Task Roles</label>
                        <span style="font-size:11px; color:var(--text-muted); background:rgba(99,102,241,.08); border:1px solid rgba(99,102,241,.15); padding:4px 10px; border-radius:7px;">
                            ✦ Members auto-loaded from selected team
                        </span>
                    </div>
                    ...
                    <div style="display:flex; flex-direction:column; gap:12px;">
                            ...
                            <div style="display:flex; flex-direction:column; gap:10px; background:rgba(255,255,255,0.02); border:1px solid var(--border-subtle); padding:14px; border-radius:12px; position:relative;">
                                ...
                                <div style="display:grid; grid-template-columns: 1fr 1fr auto; gap:12px; align-items:center;">
                                    <div>
                                        ...
                                        {{-- Hidden input submits the employee_id --}}
                                        <input type="hidden" :name="'members['+index+'][employee_id]'" :value="member.employee_id">
                                        {{-- Show name as read-only chip --}}
                                        <div class="form-input" style="font-size:12.5px; padding:6px 10px; display:flex; align-items:center; gap:6px; background:rgba(99,102,241,.05); cursor:default;">
                                            ...
                                            <span x-text="member.name || 'Unknown'"></span>
                                        </div>
                                    </div>
                                    <div>
                                ...
                                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:12px;">
                                    <div>
                                        ...
                                        <input type="date" :name="'members['+index+'][started_at]'" x-model="member.started_at" class="form-input" style="font-size:12px; padding:5px 10px; color-scheme:light;">
                                    </div>
                                    <div>
                                        <label class="form-label" style="font-size:10px; margin-bottom:4px;">Completed At</label>
                                        <input type="date" :name="'members['+index+'][completed_at]'" x-model="member.completed_at" class="form-input" style="font-size:12px; padding:5px 10px; color-scheme:light;">
                                    </div>
                                </div>
                            </div>
                        </template>
                        ...
                        <div x-show="members.length === 0" style="text-align:center; padding:24px; border:1px dashed var(--border-subtle); border-radius:10px; color:var(--text-muted); font-size:13px;">
                            Select a team above — its members will appear here automatically.
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:18px;">
                <div>
                    ...
                    <select name="status" id="status" required class="form-input" style="cursor:pointer;">
                        <option value="Pending"     {{ old('status') == 'Pending'     ? 'selected' : '' }} style="color:#000;">Pending</option>
                        <option value="In Progress" {{ old('status') == 'In Progress' ? 'selected' : '' }} style="color:#000;">In Progress</option>
                        <option value="Completed"   {{ old('status') == 'Completed'   ? 'selected' : '' }} style="color:#000;">Completed</option>
                    </select>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:18px;">
                <div>
                    ...
                    <input type="number" step="0.1" name="estimated_hours" id="estimated_hours"
                        value="{{ old('estimated_hours', 0) }}" required
