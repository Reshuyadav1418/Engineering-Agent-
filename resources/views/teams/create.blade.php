...
<div class="page-container animate-fade-up" style="max-width:800px;">
    ...
    <div style="display:flex; align-items:center; gap:12px; margin-bottom:24px;">
        ...
        <div>
            ...
            <p class="page-subtitle">Configure a new engineering team squad and assign leadership/members.</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('teams.store') }}">
            ...
            <div style="display:flex; flex-direction:column; gap:20px;">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="e.g. Frontend Core, Sentinel Squad" class="form-input" required>
                    @error('name')
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="Define the team focus, goals, or scope…" class="form-input" style="resize:vertical;">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="team_lead_id" name="team_lead_id" class="form-input">
                        ...
                        @foreach($employees as $emp)
                            <option value="{{ $emp->id }}" {{ old('team_lead_id') == $emp->id ? 'selected' : '' }}>
                                {{ $emp->name }} ({{ $emp->role }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div x-data="{ members: @js(old('members', [])) }">
